...
def test_get_wallet_address_singlesig_legacy_v1_ui(client: Client, comm: SpeculosClient,
    ...
    x.start()
    assert client.get_wallet_address(wallet, None, 0,  0, True) == "qc8MCVJwQZTNdM2JhXhBs41fxZzZ8HN9cR"
    x.join()
    ...
    if model == "stax":
        ...
        x = threading.Thread(target=ux_thread)
    x.start()
    assert client.get_wallet_address(wallet, None, 1, 15, True) == "qbHdLA6nhsuNTvPoWZZm5RHSfXcLerpHLd"
    x.join()
